...
import { SaleStatisticsOption, } from "../types";
import { monthNames } from "../utils";
import { Product } from "../models/product/product.model";
import { ProductDto } from "../dtos/product/product.dto";
import { Category } from "../models/category/category.model";
class StatisticsService {
    async getSalesStatistics(option, year) {
    ...
    async getGeoSalesStatistics(year) {
        const { maxDate, minDate } = await this.getMinMaxSalesDates();
        let sales = await Sale.find({ deleted: false }).populate("customer", "country");
        const data = [];
    ...
    async getCategoriesSalesStatistics(option, year) {
        const sales = await Sale.find({ deleted: false });
        const { maxDate, minDate } = await this.getMinMaxSalesDates();
        const data = [];
        ...
        if (option === SaleStatisticsOption.ByMonth) {
            ...
            for (let i = 0; i < currentYearSales.length; i++) {
                ...
                for (const product of currentYearSales[i].products) {
                    const productDb = await Product.findOne({ _id: product.product }).populate("categories");
                    if (productDb) {
    ...
    async getMonthlyStatistics() {
        ...
        const currentMonthSales = await Sale.aggregate(currentMonthPipeline);
        ...
        const previousMonthSales = await Sale.aggregate(previousMonthPipeline);
        let monthlyChange = 0;
    ...
    async getWeeklyStatistics() {
        ...
        const currentWeekSales = await Sale.aggregate(currentWeekPipeline);
        const previousWeekPipeline = [
            {
                $match: {
                    date: {
        ...
        const previousWeekSales = await Sale.aggregate(previousWeekPipeline);
        let weeklyChange = 0;
